"""
Shows server error on insert of QE payload when wrong underlying BSON type is used.
"""
...
def main():
    ...
    # Create encrypted payload with a string (but field expects an int):
    insert_payload = client_encryption.encrypt("foobar", contention_factor=8, algorithm=Algorithm.INDEXED, key_id=key_id)
    coll.insert_one({"secret": insert_payload})
    # Gets server error: "Queryable Encryption encrypted value has wrong type"
